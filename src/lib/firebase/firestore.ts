...
import { Book, Category, BooksByCategory, BooksFilter, PaginationOptions } from "@/types/firestore.type";
 ...
 */
export async function getAllBooks(options?: PaginationOptions): Promise<Book[]> {
    try {
...
export async function getBooksByCategory(categoryId: string): Promise<Book[]> {
    try {
        // D'abord vérifier la vue dénormalisée (cache)
        const cacheRef = doc(db, 'booksByCategory', categoryId);
        const cacheSnap = await getDoc(cacheRef);

        if (cacheSnap.exists()) {
            const data = cacheSnap.data() as BooksByCategory;
            const lastUpdated = data.lastUpdated as Timestamp;

            // Si le cache a moins de 1 heure, l'utiliser
            const oneHourAgo = Date.now() - (60 * 60 * 1000);
            if (lastUpdated.toMillis() > oneHourAgo) {
                console.log('✅ Cache Firestore utilisé');
                return data.books;
            }
        }
 ...
 */
async function updateCategoryCache(categoryId: string, books: Book[]): Promise<void> {
    try {
        const cacheRef = doc(db, 'booksByCategory', categoryId);
 ...
 */
export async function createBook(book: Omit<Book, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
        const booksRef = collection(db, 'books');
 ...
 */
export async function getFeaturedBooks(limitCount: number = 10): Promise<Book[]> {
    try {
        const booksRef = collection(db, 'books');
        const q = query(
            booksRef,
